...
def prepare_to_run(capacity, start_at, method, options):
    ...
    for sectionnumber in range(1, sections + 1):
        print()
        print(bcolors.INFO + 'handling sipp simulator server_' + str(sectionnumber) + ':' + bcolors.RESET)
        ...
        if options == 'comm':
            check_sipp_server=sipp_server_options(sipp_host, sipp_user, sipp_password, '', remote_path, '')
            if check_sipp_server == 'communication error':
                print(bcolors.FAIL2 + '- sipp server - communication error' + bcolors.RESET)
                return()
            else:
                print(bcolors.INFO2 + '- sipp server - communication check' + bcolors.RESET)

        if options == 'jobs':
            check_sipp_jobs = sipp_server_options(sipp_host, sipp_user, sipp_password, '', remote_path, 'jobs')
            if bool(check_sipp_jobs) is True:
                print(bcolors.FAIL2 + '- sipp server - active job(s) detected' + bcolors.RESET)
                return()
            else:
                print(bcolors.INFO2 + '- sipp server - active job(s) check' + bcolors.RESET)

        if options == 'terminate':
            sipp_server_options(sipp_host, sipp_user, sipp_password, '', remote_path, 'terminate')
            print(bcolors.INFO2 + '- sipp server - active job(s) were terminated' + bcolors.RESET)
        
        if options == 'pack':
            sipp_server_options(sipp_host, sipp_user, sipp_password, '', remote_path, 'pack')
            print(bcolors.INFO2 + '- sipp server - packing all log files to zip file' + bcolors.RESET)

        if options == 'clean':
            sipp_server_options(sipp_host, sipp_user, sipp_password, '', remote_path, 'clean')
            print(bcolors.INFO2 + '- sipp server - clean all log files' + bcolors.RESET)
...
method = 'intra'
options = 'create'
prepare_to_run(capacity, start_at, method, options)
